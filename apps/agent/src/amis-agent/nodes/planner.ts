...
const PLAN_TASKS_SCHEMA = z.object({
  tasks: z
    ...
    .max(5)
    .describe(
      "AMIS 任务规划列表。原则：将页面拆解为可视化的“积木块”（如：1.顶部统计卡片 2.搜索过滤栏 3.核心数据表格 4.详情弹窗），以便逐步流式渲染。",
    ),
});
...
${failedTasks.map((t) => `- 计划: ${t.description}\n  错误提示: ${t.errorMessage}`).join("\n")}`;
  ...
  try {
    const output = await modelWithStructuredOutput.invoke([
      new SystemMessage(
        "你是一个 amis 页面规划专家。你必须调用 generate_amis_tasks 工具。请务必结合工具参数描述中的组件索引、粗粒度聚合原则和默认静态数据原则来输出方案。",
      ),
