...
export async function validator_node(
  ...
  if (!task.result) {
    ...
    if (task.status === "failed" || task.status === "json_error")
      return { tasks };
  ...
  try {
    ...
    if (dataDeps.length > 0) {
      ...
      const missingFields = dataDeps.filter(
        (field) => !jsonStr.includes(field),
      );
      ...
      if (missingFields.length > 0) {
        ...
        throw new Error(
          `生成配置未包含必需的数据字段: ${missingFields.join(", ")}。请确保使用了数据绑定语法（如 \${${missingFields[0]}}）。`,
        );
      }
    }
    ...
    // --- 构建预览 Schema 并推送 ---
    // 收集之前已完成的任务结果 + 当前结果
    const completedResults = updatedTasks
    ...
    const partialSchema = {
      ...
      title: "正在构建中...",
      body: [
        ...completedResults.map((comp, idx) => ({
          type: "wrapper",
          className:
            "border-2 border-dashed border-green-200 p-2 mb-2 rounded relative",
          body: [
            {
              type: "tpl",
    ...
    const updatedTasksError = [...tasks];
    updatedTasksError[currentIndex] = {
      ...updatedTasksError[currentIndex],
      status: "json_error",
